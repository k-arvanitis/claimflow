...
CMS1500_TEMPLATE_URL = "https://www.cms.gov/medicare/cms-forms/cms-forms/downloads/cms1500.pdf"
...
FEMA_IHP_API = "https://www.fema.gov/api/open/v2/IndividualsAndHouseholdsProgramValidRegistrations"
...
CMS_SAMPLE_EOB_PDF = "https://www.cms.gov/files/document/11819-sample-explanation-benefits-508.pdf"
...
def _download_file(url: str, dest: Path) -> None:
    req = Request(url, headers={"User-Agent": "Mozilla/5.0 (ClaimFlow eval data fetch)"})
...
def register_health_lookups() -> None:
    ...
    if not path.exists():
        print("  icd10.csv not found — run `uv run python scripts/download_lookups.py` first")
        return
...
def download_real_hcpcs() -> Path:
    ...
    if not out_path.exists():
        ...
        req = Request(hcpcs_url, headers={"User-Agent": "Mozilla/5.0 (ClaimFlow eval data fetch)"})
        with urlopen(req, timeout=60) as resp:
            zip_bytes = resp.read()
    ...
    print(f"  registered real HCPCS Level II: {out_path} ({n_codes} codes, sha256 {sha256[:12]}...)")
    return out_path
...
def download_nppes_sample() -> Path:
    ...
    print(f"NPPES sample saved: {out_path} ({len(all_results)} records, sha256 {sha256[:12]}...)")
    return out_path
...
def download_fema_ihp_sample() -> Path:
    ...
    print(f"FEMA IHP sample saved: {out_path} ({len(records)} records, sha256 {sha256[:12]}...)")
    return out_path
...
def download_sba_loan_samples() -> Path:
    ...
    if not sba7a_path.exists():
        print(f"Downloading {SBA_7A_FY2020_PRESENT_URL} (full file, ~150MB — server ignores Range) ...")
    ...
    if not ppp_path.exists():
        print(f"Streaming a small sample from {SBA_PPP_OVER_150K_URL} (full file is ~450MB) ...")
    ...
    print(f"  SBA PPP sample: {ppp_path} ({n_ppp_rows} loan records, sha256 {sha_ppp[:12]}...)")
    return out_dir
