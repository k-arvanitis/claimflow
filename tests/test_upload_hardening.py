...
def test_upload_rejects_unsupported_extension():
    with TestClient(app) as client:
        resp = client.post(
            "/packages",
            files=[
                ("files", ("malware.exe", io.BytesIO(b"x"), "application/octet-stream"))
            ],
        )
...
def test_upload_rejects_oversized_file_and_leaves_no_trace(monkeypatch, tmp_path):
    ...
    with TestClient(app) as client:
        resp = client.post(
            "/packages",
            files=[
                ("files", ("big.pdf", io.BytesIO(b"%PDF-1.4" * 100), "application/pdf"))
            ],
        )
    assert resp.status_code == 400
    ...
    assert [
        p for p in tmp_path.iterdir() if p.name != "test.db"
    ] == []  # no orphaned package directory
...
def test_upload_rejects_no_files_leaves_no_trace_and_no_db_row(monkeypatch, tmp_path):
    ...
    assert (
        db.list_packages(session).__len__() == 0
        if hasattr(db.list_packages(session), "__len__")
        else True
    )
    session.close()
    ...
    with TestClient(app) as client:
        resp = client.post(
            ...
            files=[
                (
                    "files",
                    ("claim.pdf", io.BytesIO(b"%PDF-1.4 first"), "application/pdf"),
                ),
                (
                    "files",
                    ("claim.pdf", io.BytesIO(b"%PDF-1.4 second"), "application/pdf"),
                ),
            ],
        )
...
def test_document_list_does_not_expose_filesystem_path():
    with TestClient(app) as client:
        create = client.post(
            "/packages", files={"files": ("claim.pdf", b"%PDF-1.4", "application/pdf")}
        )
...
def test_page_render_succeeds_for_legitimate_document(tmp_path, monkeypatch):
    ...
    with patch("api.main.build_graph", return_value=mock_graph):
        with TestClient(app) as client:
            ...
            document_id = client.get(f"/packages/{package_id}/documents").json()[0][
                "document_id"
            ]
