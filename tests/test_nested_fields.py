...
def test_row_field_persists_parent_field(session):
    ...
    rows = db.create_extracted_fields(
        session,
        "run1",
        [
            {
                "name": "service_lines",
                "value": [{"cpt_code": "99213"}],
                "confidence": 0.9,
                "grounded": True,
                "valid": True,
                "field_status": "found",
...
def test_row_field_reviewed_independently_of_parent(session_factory):
    ...
    with TestClient(app) as client:
        resp = client.post(
            ...
            json={
                "action": "edit",
                "corrected_value": {"cpt_code": "99214"},
                "reviewer": "alice",
            },
        )
    ...
    action = (
        ...
        .filter_by(extraction_run_id="run1", field_name="service_lines[0]")
        .one()
    )
...
def test_evidence_unavailable_for_scalar_list_item_without_row_scoring(session_factory):
    ...
    assert len(fields) == 1
    assert (
        fields[0]["parent_field"] is None
    )  # no row-level breakdown exists for this field
