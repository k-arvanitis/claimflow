...
def test_eob_classifies_by_keyword():
    assert (
        _classify_doc_type("This is an Explanation of Benefits. This is not a bill.")[0]
        == "eob"
    )
...
def test_medicare_summary_notice_classifies_distinctly():
    assert (
        _classify_doc_type("Your Medicare Summary Notice (MSN) for this quarter")[0]
...
def test_declarations_page_classifies():
    assert (
        _classify_doc_type("Homeowners Policy Declarations Page — Coverage A Dwelling")[
            0
        ]
        == "declarations_page"
    )
...
def test_sba_form_413_classifies():
    assert (
        _classify_doc_type("Personal Financial Statement — SBA Form 413")[0]
        == "sba_form_413"
    )
    assert (
        _classify_doc_type(
...
def test_sba_form_2202_classifies():
    assert (
        _classify_doc_type("SBA Form 2202 Schedule of Liabilities")[0]
        == "sba_form_2202"
    )
...
def test_classification_only_docs_route_to_supporting_subtype():
    assert (
        _classify_doc_type("Prior Authorization required before this procedure")[0]
        == "prior_authorization_letter"
    )
    assert (
        _classify_doc_type("UB-04 CMS-1450 uniform billing form")[0] == "ub04_cms1450"
    )
    assert (
        _classify_doc_type("Roof inspection report attached")[0]
        == "roof_inspection_report"
    )
    assert _classify_doc_type("Fire Department Report #12345")[0] == "fire_report"
    assert (
        _classify_doc_type("Articles of Incorporation filed with the state")[0]
...
def test_classify_doc_type_returns_reason():
    ...
    doc_type, reason = _classify_doc_type(
        "This is an Explanation of Benefits. This is not a bill."
    )
    assert doc_type == "eob"
    assert reason is not None

    doc_type, reason = _classify_doc_type(
        "completely unrecognizable text with no keywords"
    )
...
def test_eob_schema_instantiation():
    from claimflow.domains.health import EOB, EOBClaim

    eob = EOB(
        patient_name="DOE JOHN",
        is_bill=False,
        claims=[EOBClaim(provider_charges=200.0, allowed_charges=150.0)],
    )
    assert eob.patient_name == "DOE JOHN"
    assert eob.is_bill is False
    assert eob.claims[0].provider_charges == 200.0


def test_eob_schema_supports_multiple_claims():
    from claimflow.domains.health import EOB, EOBClaim
    ...
    eob = EOB(
        ...
        is_bill=False,
        claims=[
            EOBClaim(claim_number="1", patient_responsibility=30.0),
            EOBClaim(claim_number="2", patient_responsibility=70.0),
        ],
    )
    assert len(eob.claims) == 2
    assert eob.claims[1].patient_responsibility == 70.0
...
def test_form_2202_schema_instantiation():
    ...
    sched = SBAForm2202(
        liabilities=[Liability(creditor_name="Bank X", current_balance=500.0)]
    )
...
def test_eob_flags_is_bill_true():
    ...
    failures = domain.validate(
        {"patient_name": "DOE JOHN", "is_bill": True, "claims": []}
    )
    ...
    failures = domain.validate(
        {
            "patient_name": "DOE JOHN",
            "is_bill": False,
            "claims": [{"provider_charges": 50.0, "allowed_charges": 100.0}],
        }
    )
    assert any(f["rule"] == "amount_consistency" for f in failures)


def test_eob_validates_each_claim_independently():
    domain = get_domain("eob")
    failures = domain.validate(
        {
            "patient_name": "DOE JOHN",
    ...
    assert len([f for f in failures if f["rule"] == "amount_consistency"]) == 1
    ...
    failures = domain.validate(
        {
            ...
            "claims": [
                {
                    "provider_charges": 200.0,
                    "allowed_charges": 150.0,
                    "plan_paid": 100.0,
    ...
    failures = domain.validate(
        {
            ...
            "policy_period_start": "01012027",
            "policy_period_end": "01012026",
        }
    )
...
def test_declarations_page_flags_date_of_loss_outside_period():
    ...
    failures = domain.validate(
        {
            "insured_name": "JOHN SMITH",
            "policy_period_start": "01012026",
            "policy_period_end": "12312026",
...
def test_form_413_flags_net_worth_arithmetic_mismatch():
    ...
    failures = domain.validate(
        {"total_assets": 100.0, "total_liabilities": 40.0, "net_worth": 999.0}
    )
...
def test_form_413_clean_net_worth_no_failures():
    domain = get_domain("sba_form_413")
    failures = domain.validate(
        {"total_assets": 100.0, "total_liabilities": 40.0, "net_worth": 60.0}
    )
    assert failures == []
    ...
    failures = domain.validate(
        {
            "liabilities": [
                {
                    "creditor_name": "Bank X",
                    "original_amount": 100.0,
                    "current_balance": 500.0,
                    "current_or_delinquent": "current",
                }
            ],
        }
    )
