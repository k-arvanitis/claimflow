...
def _seed_field_with_evidence(session, evidence=None):
    ...
    field = db.ExtractedField(
        ...
        name="patient_name",
        value_json=json.dumps("DOE JOHN"),
        confidence=0.8,
        grounded=True,
        valid=True,
        field_status="review",
    ...
    session.refresh(
        field
    )  # warm attributes while still attached, before caller closes the session
    return field


def test_evidence_response_has_full_contract_shape(session_factory):
    session = session_factory()
    field = _seed_field_with_evidence(
        session,
        {
            "page": 1,
...
def test_evidence_missing_geometry_returns_null_bbox_and_unavailable_flag(
    session_factory,
):
    session = session_factory()
...
def test_evidence_malformed_bbox_rejected_to_null(session_factory):
    ...
    field = _seed_field_with_evidence(
        session,
        {
            ...
            "text": "x",
            "bbox": [1.0, 2.0],
            "block_type": "paragraph",  # wrong length
        },
    )
    session.close()
...
def test_evidence_points_to_correct_field_after_reprocess_creates_new_run(
    session_factory,
):
    session = session_factory()
    session.add(db.Package(id="pkg1", status="review_ready"))
    ...
    fields1 = db.create_extracted_fields(
        session,
        run1.id,
        [
            {
    ...
    fields2 = db.create_extracted_fields(
        session,
        run2.id,
        [
            {
    ...
    assert (
        new_resp.json()["quote"] == "new quote"
    )  # attempt 2's evidence is independent
