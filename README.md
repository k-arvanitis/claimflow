![Python 3.13+](https://img.shields.io/badge/python-3.13+-blue.svg) ![LangGraph](https://img.shields.io/badge/LangGraph-agent-1C3C3C.svg) ![FastAPI](https://img.shields.io/badge/FastAPI-backend-009688.svg) ![Next.js](https://img.shields.io/badge/Next.js-frontend-black.svg) ![Qdrant](https://img.shields.io/badge/Qdrant-vector%20search-DC244C.svg) ![doc-intel](https://img.shields.io/badge/doc--intel-extraction%20layer-informational.svg) ![CI](https://github.com/k-arvanitis/claimflow/actions/workflows/ci.yml/badge.svg)
...
```
                          Health · Property · Loan

  ┌──────────┐     ┌──────────┐     ┌──────────┐     ┌──────────┐     ┌──────────┐
  │  Ingest  │────▶│ Extract  │────▶│ Validate │╌╌╌▶│ Retrieve │────▶│  Review  │
  │  deter-  │     │  LLM ✦   │     │  deter-  │ fail│  LLM ✦   │     │  deter-  │
  │ ministic │     │confidence│     │ ministic │ ures│ Qdrant + │     │ ministic │
  │OCR fall- │     │per field │     │  domain  │     │ reranker │     │threshold │
  │   back   │     │          │     │  rules   │     │          │     │ routing  │
  └──────────┘     └──────────┘     └────┬─────┘     └──────────┘     └────┬─────┘
                                          └╌╌╌╌╌╌ no failures (skip retrieve) ╌╌╌╌╌╌▶┘
                                                                                │
                                                                    ┌───────────┼───────────┐
                                                                    ▼           ▼           ▼
                                                               [approved]  [flagged]  [escalated]

  ✦ LLM node (2 of 5)    plain = deterministic node (3 of 5)
```
...
**`policy_required` gates policy retrieval.** It's `True` only when a domain's `question_templates` has a matching entry for that rule (or for the CMS-1500 NPI-format special case) — i.e. when there's an actual policy citation to retrieve. `retrieve_node` filters to `policy_required=True` failures before calling Qdrant at all.
...
24 endpoints. OpenAPI schema is auto-generated by FastAPI and served at `/docs` (Swagger UI) and `/openapi.json`; `tests/test_openapi_contract.py` checks the schema matches the actual response models exactly (it asserts the full route set, so this count can't silently drift from the code).
...
ClaimFlow cannot run fully on local models by default. What's actually local vs external, verified against the code:

- **OCR is local by default and requires no external API.** `doc-intel`'s OCR path (`OCR_PROVIDER=paddleocr_vl`, fallback `lighton,tesseract` — `doc_intel/config.py`) runs PaddleOCR-VL and Tesseract on-machine; LightOn also expects a locally-served vLLM instance, not a hosted API.
- **Structured extraction and non-CMS policy synthesis default to Anthropic's hosted API.** `Settings.doc_intel_provider` defaults to `"anthropic"` (`src/claimflow/config.py`), and `nodes/retrieve.py`'s `_get_llm_client()` uses `anthropic.Anthropic` with `settings.anthropic_api_key` whenever `doc_intel_provider == "anthropic"`.
- **Both can be pointed at a local model.** Setting `DOC_INTEL_PROVIDER=openai` plus `DOC_INTEL_LLM_BASE_URL` (e.g. a local vLLM endpoint) switches both structured extraction and LLM-synthesized policy retrieval (`retrieval_mode="llm_synthesis"`) to that endpoint via an OpenAI-compatible client (`nodes/retrieve.py`). CMS-1500's `retrieval_mode="official_deterministic"` never calls an LLM regardless of this setting.
- **The runtime BYOK override** (`llm_credentials.py`) only lists Groq, OpenRouter, and OpenAI — all hosted APIs, not a local option — so switching to local requires editing `.env`, not the Settings UI.

Bottom line: local-only operation is possible (OCR always; extraction and retrieval reasoning via `DOC_INTEL_PROVIDER=openai` + a local `DOC_INTEL_LLM_BASE_URL`) but is not the out-of-the-box configuration — the defaults call Anthropic's API for extraction and non-CMS retrieval synthesis.
...
Built by Konstantinos Arvanitis — AI engineer specialising in LangGraph agents and document-intelligence pipelines.
