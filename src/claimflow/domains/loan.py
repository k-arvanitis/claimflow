...
_SUPPORTING_TYPES = {
    ...
    "bank_statement": {"account summary", "beginning balance", "ending balance", "statement period"},
    "balance_sheet": {"balance sheet", "total assets", "total liabilities", "shareholders equity"},
    "income_statement": {"income statement", "profit and loss", "net income", "total revenue"},
    "id_document": {"driver license", "passport", "date of birth", "social security number"},
    "supporting_exhibit": {"exhibit a", "exhibit b", "attachment", "supporting documentation"},
    # Classification-only — recognized and routed for manual triage, no extraction pipeline.
    "profit_loss_statement": {"profit & loss", "p&l statement", "statement of profit and loss"},
    "debt_schedule": {"debt schedule", "schedule of debts", "outstanding debt summary"},
    "business_license": {"business license", "license number", "certificate of occupancy"},
    "articles_of_incorporation": {"articles of incorporation", "certificate of incorporation", "articles of organization"},
...
LOAN = Domain(
    doc_type="loan",
    keywords={"sba loan application", "small business administration", "business loan application", "loan application", "loan request form"},
    spec=_SPEC,
    validate=_validate,
...
class SBAForm413(BaseExtraction):
    applicant_name: str | None = Field(default=None, description="Person completing the statement")
    business_name: str | None = Field(default=None, description="Associated business name, if any")
    as_of_date: str | None = Field(default=None, description="Statement 'as of' date MMDDYYYY")
    cash_on_hand: float | None = Field(default=None, description="Cash on hand and in banks")
    savings_accounts: float | None = Field(default=None, description="Savings account balances")
    ira_retirement_accounts: float | None = Field(default=None, description="IRA/retirement account balances")
    accounts_notes_receivable: float | None = Field(default=None, description="Accounts and notes receivable")
    life_insurance_cash_surrender: float | None = Field(default=None, description="Life insurance cash surrender value")
    stocks_bonds: float | None = Field(default=None, description="Stocks and bonds value")
    ...
    other_personal_property: float | None = Field(default=None, description="Other personal property value")
    total_assets: float | None = Field(default=None, description="Total assets — the form's own printed total")
    ...
    installment_accounts_auto: float | None = Field(default=None, description="Installment account — automobile")
    installment_accounts_other: float | None = Field(default=None, description="Installment account — other")
    loan_on_life_insurance: float | None = Field(default=None, description="Loan against life insurance")
    mortgages: float | None = Field(default=None, description="Mortgages on real estate")
    unpaid_taxes: float | None = Field(default=None, description="Unpaid taxes")
    other_liabilities: float | None = Field(default=None, description="Other liabilities")
    total_liabilities: float | None = Field(default=None, description="Total liabilities — the form's own printed total")
    net_worth: float | None = Field(default=None, description="Net worth — the form's own printed total")
    annual_income: float | None = Field(default=None, description="Annual income")
    contingent_liabilities: float | None = Field(default=None, description="Contingent liabilities")
...
def _validate_form_413(data: dict) -> list[ValidationFailure]:
    ...
    amount_fields = [
        ...
        "life_insurance_cash_surrender", "stocks_bonds", "real_estate", "automobiles",
        "other_personal_property", "total_assets", "accounts_payable", "notes_payable",
        "installment_accounts_auto", "installment_accounts_other", "loan_on_life_insurance",
        "mortgages", "unpaid_taxes", "other_liabilities", "total_liabilities", "annual_income",
    ...
    total_assets, total_liabilities, net_worth = (
        data.get("total_assets"), data.get("total_liabilities"), data.get("net_worth"),
    )
...
class Liability(BaseExtraction):
    ...
    original_amount: float | None = Field(default=None, description="Original loan/liability amount")
    original_date: str | None = Field(default=None, description="Date the liability originated MMDDYYYY")
    current_balance: float = Field(description="Current outstanding balance")
    maturity_date: str | None = Field(default=None, description="Maturity date MMDDYYYY")
    payment_amount: float | None = Field(default=None, description="Periodic payment amount")
    secured_by: str | None = Field(default=None, description="Collateral securing the liability")
    current_or_delinquent: str | None = Field(default=None, description="'Current' or 'Delinquent' status")


class SBAForm2202(BaseExtraction):
    liabilities: list[Liability] = Field(description="Schedule of liabilities line items")
    total_current_balance: float | None = Field(default=None, description="Total current balance — form's own printed total")
...
def _validate_form_2202(data: dict) -> list[ValidationFailure]:
    ...
    for liability in liabilities:
        ...
        if (
            original_amount is not None and current_balance is not None
